...
def get_activities():
		response = requests.get("https://www.strava.com/api/v3/athlete/activities", headers={'Accept': 'application/json', 'Authorization': f'Bearer {current_user.token}'})
...
def get_activity(id):
		response = requests.get(f"https://www.strava.com/api/v3/activities/{quote_plus(id)}", headers={'Accept': 'application/json', 'Authorization': f'Bearer {current_user.token}'})
		if response.status_code != 200:
...
def activity(activity_id):
		...
		return render_template('activity.html', activity=get_activity(activity_id), coordinate_str=coordinate_str)
